//该版本仅在原答案上修改了write函数，以测试write函数是否可以改成read函数格式，答案为不能
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        else if (args[i].cpid == 0)
        {
            ...
            while ((size = write(args[i].pipefd[1], args[i].bins->data + pos / sizeof(int), sizeof(int) * args[i].bins->size)) > 0)
            {
        ...
        else
        {
            ...
            while ((size = read(args[i].pipefd[0], args[i].bins->data + pos / sizeof(int), sizeof(int) * args[i].bins->size)) > 0)
            {
                printf("child[%d] pipe pos=%d size=%d\n", i, pos, size);
                pos += size;
            };
    ...
    for (int i = 0; i < 4; i++)
    {
        waitpid(args[i].cpid, &args[i].status, 0);
    }
